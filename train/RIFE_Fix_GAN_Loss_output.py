...
class Model:
    def __init__(self, local_rank=0):
        ...
        self.metricnet = DDP(self.flownet, device_ids=[local_rank], output_device=local_rank)
    ...
    def update(self, imgs, gt, learning_rate=0, training=True, timestep=0.5):
        ...
        mergedlpips, gtlpips = simple_color_aug.reverse_augment(merged), simple_color_aug.reverse_augment(gt)
        
        loss_l1 = self.l1_loss(mergedl1 - gt) 
        loss_lpips = (self.lpips.forward((mergedlpips - 0.5) / 0.5, (gtlpips - 0.5) / 0.5).mean((1,2,3))).mean()
        loss_gan = self.gan_loss(self.net_d(merged), True, is_disc=False)

        if training:
            loss_G = loss_l1 * 2 + loss_lpips + loss_gan
            loss_G.backward()
            self.optimG.step()
        ...
        return merged, torch.cat((flow01, flow10), 1), metric0, metric1, loss_l1, loss_lpips, torch.mean(real_d_pred.detach()), torch.mean(fake_d_pred.detach())
